...
from module_3.layout_option_model import Ui_MainWindow  as uicmain
from module_3.database_processing_pyuic5 import Process_Model 
from module_3.manage_signal_transmission_legion import Ui_MainWindow as AutomationInterface
from module_3.signal_processing_pyuic5 import DataProcessSignal 
from module_4.hik_vision.pyuic5.hik_mvs_setting_device_list import Ui_MainWindow as Camera_Device
import sys
...
class Ui_MainWindow(object):
    ...
    def __init__(self,i_code,db,connect_db): 
        ...
        self.trigger_file_path = r"C:\gui_part_legion\module_3\trigger.txt"
    ...
    def _create_model_tabs(self,models,camera_name,camera_id):
        ...
        for model_index, _ in enumerate(models):
            ...
            self.ui_child.pushButton_3.clicked.connect(lambda _,unit=self.processor:unit.browse_folder_activity_history())
    ...
    def update_database(self):
        ...
        for index, record in enumerate(camera_records):
            ...
            self.Automation.pushButton_applyshotxmodel.clicked.connect(lambda _,unit=self.DataProcessSignal:unit.save_shotxmodel_to_db(self.cursor,self.connect_db,item_code_id))
            self.Automation.pushButton_applysignalplc.clicked.connect(lambda _,unit=self.DataProcessSignal:unit.save_values_signal_to_db(self.cursor,self.connect_db,item_code_id))
            ...
            init_camera = QtWidgets.QMainWindow()
            self.layer_init_camera = Camera_Device()
            self.layer_init_camera.setupUi(init_camera)
            self.tabWidget_2.addTab(init_camera, f"Camera Device")
            self.product_code_forms.append(init_camera)
    ...
    def check_trigger_once(self):
        ...
        for key, values in self.built_in_object.items():
            ...
            trigger_read_value_on = signals['read_value_on']
            trigger_read_value_off = signals['read_value_off']
            # try:
            if int(self.read_plc(f'{address}{trigger}')) == int(trigger_read_value_on):
                self.t1 = time.time()
                print(f'{address}{trigger} - {trigger_read_value_on} - {trigger_read_value_off}')
                self.write_plc(f'{address}{trigger}', trigger_read_value_off)  
                self.task.put((key, values))
                elapsed = time.time() - self.t1
                result_text = f'RESULT: {elapsed:.2f}'
                print(result_text)
            # except Exception as e:
    ...
    def handle_triggered_camera(self, key, values):
        ...
        print('counter: ',self.counters[key])
        ...
        for (shot, model_full), pyuic5 in pyuic_dict.items():
            ...
            result_image_rgb, flag = self.processor._engine(image_rgb,450, 450,model_loader,pyuic5)

            # result_image_rgb, flag = self.engine_outdoor(image_rgb, model_loader, 450, 450)

            # elapsed = time.time() - self.t1
            elapsed = '0'
            result_text = f'RESULT: {"OK" if flag else "NG"}'
        ...
        print(f'shot_complete : {elapsed:.2f}')
