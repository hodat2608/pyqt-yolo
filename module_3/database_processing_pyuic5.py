...
from module_3.layout_option_model import COLUMN_MAPPING
import time
...
class Process_Model:
    ...
    def browse_folder_activity_history(self):
            ...
            if folder:
                self.uic.lineEdit_4.setText(folder)
    ...
    def save_values_injection_safe(self, cursor, connector):
        try:
            ...
            activity_history = self.uic.lineEdit_4.text().strip()
            device_load_model = self.uic.comboBox_device_gpu.currentText()
            ...
            if weight_path and weight_path.endswith('.pt'):
                cursor.execute("""
                    ...
                    (Weight_Path, Activity_History_Path, Conf_Value, Size_Value, Device, model_id)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (weight_path, activity_history, conf, size, device_load_model ,model_id))
    ...
    def collect_device(self):
        ...
        for i in range(devices):
            self.uic.comboBox_device_gpu.addItem(f'cuda:{i}-{torch.cuda.get_device_name(i)}')

    def load_parameter_injection_safe(self, camera_id, model, cursor, connect_db, tabWidget, ui_instances):
        try:
            weight_path = None
            cursor.execute("""
                SELECT Weight_Path, Activity_History_Path ,Conf_Value, Size_Value, Device
                FROM WEIGHTS
            ...
            if result:
                weight_path, activity_history, conf_value, size_value, device = result[0]
                ...
                self.uic.lineEdit_4.setText(activity_history)
                self.uic.comboBox_device_gpu.setCurrentText(device)
        ...
        if weight_path and weight_path.endswith('.pt'):
            ...
            self.model_loader = torch.hub.load('./yolov5', 'custom', path=weight_path, source='local', force_reload=False).to(device=device.split('-')[0])
    ...
    def engine(self,image,width_widget,height_widget):
        ...
        index_ng = []
        flag = True
        for item in range(len(result.index)):
            width = result['xmax'][item] - result['xmin'][item]
            height = result['ymax'][item] - result['ymin'][item]
            confidence = result['confidence'][item]
            label_name = result['name'][item]
                ...
                if label_item and label_item.text().strip() == label_name:  
                    ...
                    if join_item and join_item.checkState() == Qt.Checked:
                        ...
                        if (confidence < (conf_item/100) or 
                            ...
                            result.drop(item, inplace=True, axis=0)
                            index_ng.append(item)
                    else:
                        if label_item and label_item.text().strip() == label_name:  
                            result.drop(item, inplace=True, axis=0)
                            index_ng.append(item)
    ...
    def _engine(self,image,width_widget,height_widget,model_loader,pyuic):
        image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) 
                ...
                if label_item and label_item.text().strip() == label_name:  
                    ...
                    if join_item and join_item.checkState() == Qt.Checked:
                        ...
                        if label_item and label_item.text().strip() == label_name:  
                            ...
                            index_ng.append(item)
        list_ok = list(result.name)
        result_image = np.squeeze(result_model.render(labels=index_ng))
        result_image_rgb = cv2.resize(result_image, (width_widget, height_widget), interpolation=cv2.INTER_AREA)
        for i in range(pyuic.model.rowCount()):
            label_item = pyuic.model.item(i,pyuic.model_column.index(self.keys_mapping[0]))
            join_item = pyuic.model.item(i,pyuic.model_column.index(self.keys_mapping[1]))
